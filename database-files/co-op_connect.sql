...
VALUES ( 2, 'Wade Wilson', 'w.wilson@northeastern.edu',3.0,
        '1', '2027', '1');
INSERT INTO coop_connect.student (id, name, email, gpa, major_id, grad_year, advised_by, past_job)
...
FROM coop_connect.interview_question
         JOIN coop_connect.job_position jp
...
FROM coop_connect.job_position jp
         JOIN coop_connect.application a
...
SELECT ca.email
FROM coop_connect.coop_advisor ca
         JOIN coop_connect.student s
...
SET status = 'Rejected'
WHERE applicant_id = '1' AND job_position_id = '1';
-- (3.4) Look at reviews for his company's positions
SELECT r.rating, r.review, c.name, jp.title
FROM coop_connect.review r
         JOIN coop_connect.job_position jp
              on r.job_position_id = jp.id
         JOIN coop_connect.company c
...
FROM coop_connect.hiring_manager hm
         JOIN coop_connect.company c
